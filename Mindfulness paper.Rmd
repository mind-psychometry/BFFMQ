...
descriptives <- rbind(des.ob,des.des,des.actw,des.nj,des.nr)
...
all.itemfit <- rbind(observe.item.fit, describe.item.fit,awareness.item.fit,awareness.item.fit,nonjudge.item.fit,nonreact.item.fit )
...
all.itemfit.sa <- rbind(observe.item.fit.2, describe.item.fit.2,awareness.item.fit.2,awareness.item.fit.2,nonjudge.item.fit.2,nonreact.item.fit.2 )
...
awareness.IIC.1 <- plot(awareness.fit.2, type = "infotrace", facet =T, main = "Awareness Item Information Curve")
...
```{r OCC-irt3, include=F}
##Option charecteristics curve
observe.OCC <- plot(observe.fit.2, type = "trace", facet =T, main = "Observe")
describe.OCC <- plot(describe.fit.2, type = "trace", facet =T, main = "Describe")
awareness.OCC <- plot(awareness.fit.2, type = "trace", facet =T, main = "Actwar")
nonjudge.OCC<- plot(nonjudge.fit.2, type = "trace", facet =T, main = "Nonjudge")
nonreact.OCC<- plot(nonreact.fit.2, type = "trace", facet =T, main = "Nonreact")

```



```{r ggiteminfo, include=F}
library(tabledown)
observe_list = list()
for (i in 1:ncol(observe.refit)) {
  p = ggiteminfo(observe.fit.2, i,6)+ggtitle(i)
  observe_list[[i]] = p
}


describe_list = list()
for (i in 1:ncol(describe.refit)) {
  p = ggiteminfo(describe.fit.2, i,6)+ggtitle(i)
  describe_list[[i]] = p
}


awareness_list = list()
for (i in 1:ncol(awareness.refit)) {
  p = ggiteminfo(awareness.fit.2, i,6)+ggtitle(i)
  awareness_list[[i]] = p
}


nonjudge_list = list()
for (i in 1:ncol(nonjudge.refit)) {
  p = ggiteminfo(nonjudge.fit.2, i,6)+ggtitle(i)
  nonjudge_list[[i]] = p
}


nonreact_list = list()
for (i in 1:ncol(nonreact.refit)) {
  p = ggiteminfo(nonreact.fit.2, i,6)+ggtitle(i)
  nonreact_list[[i]] = p
}

```

```{r plot-saveloop, eval=FALSE, include=FALSE}
for (i in 1:ncol(observe.refit)) {
  
  temp_plot = ggiteminfo(observe.fit.2, i,6)
  
  ggsave(temp_plot, file=paste0("Figures/OBSERVE_", i,".png"), width = 14, height = 10, units = "cm")
}
...
Table \@ref(tab:IRT-tab1) summarizes the item discrimination and discrimination parameters of retained items with their item fit indices. @RN1275 suggested the optimum range of item discrimination,a is .5 $\leq$ a $\leq$ 2. All items except item:38 and 39 were within this suggested limits. 
...
cowplot::plot_grid(observe.OCC, describe.OCC, awareness.OCC, nonjudge.OCC, nonreact.OCC, labels = "AUTO",ncol=2, label_size = 8,align = "v")
```
...
observe.per.fit <- ggplot(observe.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+geom_vline(xintercept = -2)+
...
des.per.fit <- ggplot(des.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+geom_vline(xintercept = -2)+
...
act.per.fit <- ggplot(act.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+geom_vline(xintercept = -2)+
  labs( y="Number of Participants", x = "Zh Statistics:Actware")+apatheme
...
nonj.per.fit <- ggplot(nonj.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+geom_vline(xintercept = -2)+
...
nonj.per.fit <- ggplot(nonr.personfit, aes(x=Zh)) + geom_histogram(binwidth=.4)+geom_vline(xintercept = -2)+
...
cowplot::plot_grid(observe.per.fit, des.per.fit, act.per.fit, nonj.per.fit, nonj.per.fit, labels = "AUTO",ncol=3, label_size = 8,align = "v")
```
...
observe.reliability <- plot(observe.fit.3, type = 'rxx', theta_lim = c(-6, 6), 
      main="" )

describe.reliability <- plot(describe.fit.3, type = 'rxx', theta_lim = c(-6, 6), 
      main="" )
awareness.reliability <- plot(awareness.fit.3, type = 'rxx', theta_lim = c(-6, 6), 
      main="" )
nonjudge.reliability <- plot(nonjudge.fit.3, type = 'rxx', theta_lim = c(-6, 6), 
      main="" )
nonreact.reliability <- plot(nonreact.fit.3, type = 'rxx', theta_lim = c(-6, 6), 
      main="" )
...
```{r TIC-fig, fig.cap= "Test information Curves: (A) Observe, (B) Describe, (C)Act with awareness, (D)Nonjudgement, (E) Nonreact", results='asis'}
cowplot::plot_grid(observe.TIC.1, describe.TIC.1, awareness.TIC.1, nonjudge.TIC.1, nonreact.TIC.1, labels = "AUTO",ncol=2, label_size = 8,align = "v")
```

```{r conrel-fig, fig.cap= "Conditional Reliability plots: (A) Observe, (B) Describe, (C)Act with awareness", results='asis'}
cowplot::plot_grid(observe.reliability, describe.reliability, awareness.reliability, nonjudge.reliability, nonreact.reliability, labels = "AUTO",ncol=2, label_size = 8,align = "v")
```
...
jpeg("Figures/wp.png", width = 800, height = 750)
...
knitr::include_graphics('Figures/wp.png')
```
...
reliability(fit.irt.1, return.total = TRUE)
...
FF.irt.H <- "Observe =~ item1+item6 + item11 + item15 + item20 + item26 + item31 + item36
          ...
          Nonjudge =~ Ritem10 +  Ritem25 + Ritem30 +Ritem35
          Nonreact =~ item4+ item9 + item19 + item21 + item24 + item29 + item33
mindfulness =~  Observe+Describe + Awareness+Nonjudge+Nonjudge+Nonreact
                                 "
...
reliability(fit.irt.H, return.total = TRUE)
...
FF.irt.1 <- "Observe =~ item6 + item11 + item15 + item20 + item26 + item31 + item36
          Describe =~  item7 + item27+ item32 + item37
          Awareness =~ Ritem5+ Ritem8 + Ritem13 + Ritem18 + Ritem23 + Ritem28 + Ritem34
          Nonjudge =~ Ritem10 +  Ritem25 + Ritem30 +Ritem35
...
```{r figcfa, fig.cap = "(A) ", fig.show="hold",fig.ncol = 2, out.width="50%",  warning=FALSE}
library(semPlot)
...
validity <- validity %>% 
  rowwise() %>% 
  mutate(observe_total = sum(item6 , item11 , item15 , item20 , item26 , item31 , item36))
...
We gathered convergent and divergent validity evidence of BFFMQ on a subset of our sample (n= `r nrow(validity)`).  We gathered convergent validity evidence based on correlational analysis. Table\@ref(tab:tabvalidity) summarized the inter correlation of the five mindfulness facets as well as correlation coefficients of these facets  with emotional intelligence [@hydePsychologicalTestManual2002], depression [@uddinDevelopmentScaleDepression2005], MAAS [@brownBenefitsBeingPresent2003; @islamValidationBanglaMindful2016], and three  measures of personality: openness, extroversion and neuroticism [@RN1278; @RN1287; @RN1288]. Table\@ref(tab:tabvalidity) indicated inter-correlations of the five facets were in general modest and significant (only three pairs: observe with actware, describe with nonjudge, and nonreact with actware were non-significant). Nonjudge was significantly negatively correlated with observe ( r = `r round(cor.tab[4,1],2)`, p < .01). Nonreact was significantly negatively correlated with   awareness ( r = `r round(cor.tab[5,4],2)`, p < .01). By judging the underlying construct and item contents of BFFMQ we expected  positive correleation among facets of BFFQ and emotional intelligence, MASS and openness since these constructs incorporate the elements of mindfulness [@baerUsingSelfReportAssessment2006]. And with neuroticism and depression negative correlations were expected as neuroticism to some extent reflect the absence of mindfulness [@baerUsingSelfReportAssessment2006]. Table\@ref(tab:tabvalidity) indicated that our directional findings of the correlations were almost consistent with our prediction. Emotional intelligence was significantly positively correlated with four facets (negatively correlated with nonjudgement, r =`r round(cor.tab[6,4],2)` p>.05 ). MASS was positively correlated with all five facets of mindfulness. Depression and neuroticism were negatively correlated with all the five facets.
